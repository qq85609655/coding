...
(function(window, undefined) {
		...
		jQuery.fn = jQuery.prototype = {
			...
			pushStack: function(elems, name, selector) {
				...
				if (jQuery.isArray(elems)) {
					...
					jQuery.merge(ret, elems); // 修改对象中的length属性
				}
			...
			sort: [].sort,
			splice: [].splice
				//wxx:如果一个对象中含有length和splice方法，那么它就可以表示成[]，形成类对象数组，只是其中的length不会自动增加。
		};
		...
		jQuery.extend({
			...
			ready: function(wait) {
				...
				if ((wait === true && !--jQuery.readyWait) || (wait !== true && !jQuery.isReady)) {
					...
					if (!document.body) {
						return setTimeout(jQuery.ready, 1); //setTimeout不会阻塞dom的加载，采用此办法继续等待
					}
					...
					console.log("this is jquery.ready");
			...
			bindReady: function() {
				...
				if (document.readyState === "complete") {
					...
					//解决DomContentLoaded事件已经发生过了的情况，通过setTimeout在jquery加载初始化后执行方法ready
				}
	...
	jQuery.Callbacks = function(flags) {
		...
		var // Actual callback list
			...
			fire = function(context, args) {
				args = args || [];
				memory = !flags.memory || [context, args]; //回调一旦fire，就会记录fire参数作为下次记忆触发的参数
				fired = true;
				firing = true;
		...
		Deferred: function(func) {
			var doneList = jQuery.Callbacks("once memory"), //一个defer对象只需要执行一次resolve或者eject方法，但是可以执行多次notify方法
				...
				promise = {
					...
					always: function() {
						//always将方法依次都加入接受和拒绝回调队列，但是在执行的时候会进行回调方法的锁定使得执行产生互斥，无法都执行
						//同时配合once属性，会产生defer对象创建以后将只能fire once
			...
			if (length > 1) {
				for (; i < length; i++) {
					if (args[i] && args[i].promise && jQuery.isFunction(args[i].promise)) {
						//对各个defer添加resolve方法探测，以及一个reject发生探测，以及各个defer的notify方法调用的探测，其参数为notify参数累加
				...
				if (!count) {
					//当传递的是多个非defer对象的时候，直接创建defer对象并resolve后返回
					deferred.resolveWith(deferred, args);
				}
			} else if (deferred !== firstParam) {
				//当传递参数为一个非defer对象，会创建一个defer并resolve后返还给外部使用
				deferred.resolveWith(deferred, length ? [firstParam] : []);
			}
			//当传参数为一个defer对象的时候直接取将约束的defer返回
			return promise;
		}
	});
	...
	jQuery.extend({
		...
		//启动动画队列的执行；此处对inprogress有写权限，其他处仅具有度权限。
	...
	(function() {
		...
		if (!document.querySelectorAll) {
			(function() {
				...
				//选择器加速：如果能力判断通过，则使用快速选择器，覆盖旧版选择器，返回的仅仅为数组
				//以下分别对选择器：.class、tag、#id等简单选择器采用了原生的dom.get*进行了加速。
	...
	var runtil = /Until$/,
		...
		// guaranteedUnique中获取的节点都是唯一节点---因为是唯一节点的子节点，所以不会出现重复，此处为不用去重，加速查询
		guaranteedUnique = {
			children: true,
			contents: true,
			next: true,
	...
	jQuery.fn.extend({
		...
		//dom操纵方法  
		//创建fragment以及对table和script标签的特殊处理
	...
	jQuery.each(["height", "width"], function(i, name) {
		jQuery.cssHooks[name] = {
			get: function(elem, computed, extra) {
				if (computed) {
					//说明：对于宽高采用了hook处理进行特性处理，get方法保证height|width只是内容区的对应值，但是set方法对应
					//的元素所属盒子模型的height|width，直接设置的style下的width|height，其最终内容区宽高与box-sizing相关。
	...
	// 动画队列也是函数队列，在动画队列中间隔插入时间清除函数进而实现时间重新获取，实现对动画的计时操作。
